...
function MoviesCard({ movie, saveMovies, savedMovies, deleteMovieCard}) {
...
const hoursAndMinutes = getHoursAndMinutes(movie.duration)
  ...
  return (
    <li className='movieCard'>
      ...
      <Route path="/movies">
        <button className={ !isSaved ? 'button movieCard__save-button' : 'button movieCard__save-button_saved'} 
      type='button' onClick={isSaved ? deleteFilm : filmSave} >{!isSaved ? `Coxpанить` : ''}</button>
      </Route>

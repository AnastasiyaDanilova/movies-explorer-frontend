  ...
  return (
    <section className='register'>
      ...
      <AuthForm title='Добро пожаловать!' buttonText='Зарегистрироваться' authText='Уже зарегистрированы?' authButtonText='Войти'
        ...
        value={name} onChange={onChangeName} type="text" pattern="^[А-Яа-яЁёa-zA-Z\s-]+$" minLength='2' maxLength='30' disabled={submitButtonDisabled} required />
        ...
        value={email} onChange={onChangeEmail} type='email' disabled={submitButtonDisabled} pattern={'^([a-z]){1,}[a-z0-9]*([.+_-]){0,1}[0-9a-z]+(@){1}([0-9a-z]+)(\\.([a-z]){2,}){1}(\\.[a-z]{2,})?$'} required />
        ...
        placeholder='Пароль' value={password} onChange={onChangePassword} type='password' minLength='2' maxLength='30' disabled={submitButtonDisabled} required />
